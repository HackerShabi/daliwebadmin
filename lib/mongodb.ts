...
export async function getDatabase(): Promise<Db> {
  ...
  return client.db('daliweb');
}

// Connect to database function for API routes
export async function connectToDatabase() {
  const client = await clientPromise;
  const db = client.db('daliweb');
  return { client, db };
}
...
export async function getDemosCollection() {
  const db = await getDatabase();
  return db.collection('demos');
}
